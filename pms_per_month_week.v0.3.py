...
print(f"第一行日志的日期为：{xls_date_end}")
print(f"最后行日志的日期为：{xls_date_start}")
pms_year = xls_date_end.year
pms_month = xls_date_end.month
pms_week = 0
pms_month_start = xls_date_start.month
if pms_month != pms_month_start:
    print(f"数据所属的月份跨月，请注意检查数据有效性")
...
def workdays_month():
    ...
    计算本月工作日
    """
    first_day_weekday, total_days = calendar.monthrange(pms_year, pms_month)
    workdays = 0
    for day in range(1, total_days+1):
        date = datetime(pms_year, pms_month, day)
        if is_workday(date):
            workdays += 1
    return workdays
...
def workdays_week():
    ...
    计算本周工作日及周数
    """
    pms_year, pms_week, weekday = xls_date_end.isocalendar()
    start_of_year = datetime(pms_year, 1, 1)
    if start_of_year.weekday() == 0:
        days_to_week_start = (pms_week - 1) * 7
    else:
...
if xls_days <= 7:
    ...
    xls_type = 'WEEK'
if 7 < xls_days <= 31:
    workdays = workdays_month()
    print(f"数据以{pms_year}年{pms_month}月为标准进行计算")
    print(f"工作日一共:{workdays}天")
    xls_type = 'MONTH'
...
# 定义后续Excel文件名
if xls_type == 'MONTH':
    file_path = f"{pms_file_path}{pms_year}-{pms_month}.xlsx"
    file_path_output = f"{pms_file_path}{pms_year}-{pms_month}-output.xlsx"

if xls_type == 'WEEK':
    file_path = f"{pms_file_path}{pms_year}-week{pms_week}.xlsx"
    file_path_output = f"{pms_file_path}{pms_year}-week{pms_week}-output.xlsx"
...
result_df['日志时长差'] = np.where(
    result_df['有效日志时长'] >= (result_df['工作日时长'] * 0.8 - result_df['请假时长']),
    '达标',
    (result_df['工作日时长'] * 0.8 - result_df['请假时长']) - result_df['有效日志时长']
)
...
if xls_type == 'MONTH':
    ...
    new_column_order = ['姓名', '工号', 'Base地', '岗位类别', '是否外包', '月份', '工作日', '工作日时长', '日志时长', '请假时长', '居家办公加日常', '有效日志时长', '有效日志占比', '项目日志时长', '项目日志占比', 'KPI参考', '排名', 'KPI有效值（0-150）', '邮箱', '备注']
    # 选择并重新排列列
...
if xls_type == 'WEEK':
    ...
    new_column_order = ['姓名', '工号', 'Base地', '岗位类别', '是否外包', '周数', '工作日', '工作日时长', '日志时长', '请假时长', '居家办公加日常', '有效日志时长', '有效日志占比', '项目日志时长', '项目日志占比', 'KPI参考', '排名', '日志时长差', '邮箱', '备注']
    # 选择并重新排列列
    result_df = result_df[new_column_order]
    # 将结果保存到Excel文件
    result_df.to_excel(file_path_output, index=False)
