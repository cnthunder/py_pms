...
xls_date = datetime.strptime(str(xls_df.iloc[1, 0]), '%Y-%m-%d %H:%M:%S').date()
xls_date_last = datetime.strptime(str(xls_df.iloc[-1, 0]), '%Y-%m-%d %H:%M:%S').date()
print(f"第一行日志的日期为：{xls_date}")
print(f"最后行日志的日期为：{xls_date_last}")
pms_year = xls_date.year
pms_month = xls_date.month
pms_week = 0
pms_month_last = xls_date_last.month
if pms_month != pms_month_last:
...
def workdays_week():
    ...
    计算本周工作日及周数
    """
    pms_year, pms_week, weekday = xls_date.isocalendar()
...
# 判断类型为周还是月
xls_days = (xls_date - xls_date_last).days + 1
